...
ssize_t get_input(info_t *info)
{
	...
	{
		z = x; /* init new iterator to current buf position */
		p = buf + x; /* get pointer for return */

		check_chain(info, buf, &z, x, len);
...
ssize_t read_buf(info_t *info, char *buf, size_t *x)
{
	...
	if (*x)
		return (0);
	r = read(info->readfdesc, buf, READ_BUF_SIZE);
	if (r >= 0)
		*x = r;
	return (r);
}
